...
stat = "[epoch:%d/batch:%d/total_step:%d] t_loss: %f, d_p: %f, d_n:%f, time:%f"
...
class TripNet(object):
    def __init__(self, sess,
        ...
        with tf.variable_scope('shared') as scope:
            ...
            if train:
                ...
                self.f_p = self.build_model(self.x_p, train=train, scope='positive')
                self.f_n = self.build_model(self.x_n, train=train, scope='negative')

        self.vars_save = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES)

        if train:
            self.loss, self.d_p, self.d_n = self.triplet_loss(self.f_q, self.f_p, self.f_n)
    ...
    def build_model(self, x, train=True, scope=None):
        with tf.name_scope(scope):
            h = tf.reshape(x, [-1, 384, 256, 3])
            h = tf.layers.conv2d(inputs=h, filters=64,
                                 kernel_size=[3, 3], padding='same',
                                 activation=tf.nn.leaky_relu, name='cv1_1')
            h = tf.layers.conv2d(inputs=h, filters=64,
                                 kernel_size=[3, 3], padding='same',
                                 activation=tf.nn.leaky_relu, name='cv1_2')
            h = tf.layers.dropout(inputs=h, rate=0.25, name='cv1_drop')
            h = tf.layers.max_pooling2d(inputs=h, pool_size=[4, 4],
                                        strides=[4, 4], name='cv2_pool')
            h = tf.layers.batch_normalization(inputs=h,
                                              training=train, name='cv2_norm')
            h = tf.layers.conv2d(inputs=h, filters=128,
                                 kernel_size=[3, 3], padding='same',
                                 activation=tf.nn.leaky_relu, name='cv2_1')
            h = tf.layers.conv2d(inputs=h, filters=128,
                                 kernel_size=[3, 3], padding='same',
                                 activation=tf.nn.leaky_relu, name='cv2_2')
            h = tf.layers.dropout(inputs=h, rate=0.25, name='cv2_drop')
            h = tf.layers.max_pooling2d(inputs=h, pool_size=[4, 4],
                                        strides=[4, 4], name='cv3_pool')
            h = tf.layers.batch_normalization(inputs=h,
                                              training=train, name='cv3_norm')
            h = tf.layers.conv2d(inputs=h, filters=256,
                                 kernel_size=[3, 3], padding='same',
                                 activation=tf.nn.leaky_relu, name='cv3_1')
            h = tf.layers.conv2d(inputs=h, filters=256,
                                 kernel_size=[3, 3], padding='same',
                                 activation=tf.nn.leaky_relu, name='cv3_2')
            h = tf.layers.dropout(inputs=h, rate=0.25, name='cv3_drop')
            h = tf.layers.max_pooling2d(inputs=h, pool_size=[4, 4],
                                        strides=[4, 4], name='cv4_pool')
            h = tf.layers.batch_normalization(inputs=h,
                                              training=train, name='cv4_norm')
            h = tf.layers.conv2d(inputs=h, filters=128,
                                 kernel_size=[3, 3], padding='same',
                                 activation=tf.nn.leaky_relu, name='cv4_1')
            h = tf.layers.flatten(inputs=h, name='flatten')
            h = tf.layers.dense(inputs=h, units=128,
                                activation=tf.nn.leaky_relu, name='cv4_fc')
        return h
    ...
    def train(self):
        ...
        log_dir = make_log_dir(self.log_dir, now)
        ...
        for epoch in range(self.epoch):
            ...
            for batch in range(batch_num):
                ...
                path = dataset[start:end]

                img_q, img_p, img_n = get_batch(self.data_dir, paths)

                _, train_summary = self.sess.run([train_step, self.summary],
                ...
                loss, d_p, d_n = self.sess.run([self.loss, self.d_p, self.d_n],
                                               feed_dict={self.x_q: img_q,
                                                          self.x_p: img_p,
                                                          self.x_n: img_n})
                print(stat % (epoch, batch, step, loss, d_p, d_n, t))
    ...
    def load(self, load_step=None):
        ...
        if ckpt:
            if load_step:
                name = 'mnist-api.model-%d' % load_step
                self.saver.restore(self.sess, os.path.join(ckpt_dir, name))
            else:
                self.saver.restore(self.sess, ckpt.model_checkpoint_path)
        else:
            raise Exception('[!] No checkpoint files are in %s' % ckpt_dir)
    ...
    def visualize(self, width=64, height=96):
        ...
        vec = np.genfromtxt(os.path.join(self.visualize_dir, "./vector.tsv"))

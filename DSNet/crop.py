...
parser = argparse.ArgumentParser(
    description='This Program Makes the List of Triplets.')
parser.add_argument("--data_dir", type=str,
                    default="./data/train", help="Specify the Data Directory.")
...
def get_biggest_idx(data):
    idx = 0
    for i, row in enumerate(data):
        if row[2] * row[3] > data[idx][2] * data[idx][3]:
            idx = i
    # print(idx)
    return idx


def get_trimed_img(img, data):
    person = []
    for row in data:
        if row[0] == "person":
            person.append(row[1:])
    try:
        person = person[get_biggest_idx(person)]
        print(person)
        left = int(person[0] - person[2] / 2)
        right = int(person[0] + person[2] / 2)
        print(left, right)
        trimed = np.zeros((img.shape[0], img.shape[1], 3), np.uint8)
        trimed[:, left:right] = img[:, left:right]
        return trimed
    except:
        return img
...
if __name__ == '__main__':
    ...
    for filename in files:
        ...
        print(yolo.result)
        cv2.imwrite(filename, cv2.resize(
            get_trimed_img(img, yolo.result), (256, 384), interpolation=cv2.INTER_CUBIC))
